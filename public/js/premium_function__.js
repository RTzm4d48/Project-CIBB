...
function select_styles(data){
    ...
    var content = "<div id='id_color_space_D' class='color_space'>"+
                    "<div class='icon_name'>"+
                        "<img src='/public/svg/premium/palette-solid.svg' alt=''>"+
                        "<p>Default</p>"+
                    "</div>"+
                    "<div class='cont_p_color'>"+
                        "<div style='background:#45A1E5' class='c'></div>"+
                        "<div style='background:#5693DB' class='c'></div>"+
                        "<div style='background:#fff' class='c'></div>"+
                        "<div style='background:#36393F' class='c'></div>"+
                        "<div style='background:#8EDF4A' class='c'></div>"+
                        "<div style='background:#36393F' class='c'></div>"+
                    "</div>"+
                    "<ul class='nav'>"+
                        "<li><img src='/public/svg/premium/ellipsis-h-solid.svg'>"+
                            "<ul>"+
                            "<li><p onclick='Default();'>Aplicar</p></li>"+
                            "</ul>"+
                        "</li>"+
                    "</ul>"+
                "</div>";
...
function Default(){
    $.ajax({
        url: '/public/ajax/premium_ctr/ajax_default.php',
        type: 'POST',
        /* data: 'id='+id, */
        dataType: "json",
        success:function(rpt){
            /* alert(rpt); */
            $('.fondo').css('background-color','#45A1E5');
            //buttons -2-
            $('.buttons').css('background-color','#5693DB');
            //line -3-
            $('.line').css('background','linear-gradient(to right,#ffffff00, '+'#fff'+'35, #ffffff00)');
            //content -4-
            $('.conteinn').css('background-color','#36393F');
            $('.op-conteinn').css('background-color','#36393F'+'98');
            $('.d-op-conteinn').css('background-color','#36393F'+'b4');
            $('.es-op-conteinn').css('background-color','#36393F'+'27');
            //join -5-
            $('.join').css('background-color','#8EDF4A');
            //content -6-
            $('.op-conteinn_p').css('background-color','#36393F'+'98');
            messagebox_2('Default','Los estilos de tu interfaz se aplicaron y restablecieron exitosamente.');
        }
    });
}
//PART 3
